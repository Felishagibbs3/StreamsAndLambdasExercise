...
public class App {
    ...
    public static void main(String[] args) {
        ...
        //created new list to hold all characters whose mass is greater than 80/ if so, added to list
        List<Character> heavyCharacters = characters.stream()
                .filter(character -> character.getMass() > 80)
                .toList();
        //looping over the heavy characters to display the list
        ...
        boolean hasBlueEyes = characters.stream()
                .anyMatch(character -> character.getEyeColor().equalsIgnoreCase("blue eyes"));
        ...
        boolean allMale = characters.stream()
                .allMatch(character -> character.getGender().equalsIgnoreCase("Leia"));
        if(allMale) {
